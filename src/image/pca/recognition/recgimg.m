...
for n=1:M
    ...
    imgmat(:,n) = im(:); % convert image to vector and store as column in matrix I
end

% calculate mean image
img_mean = mean(imgmat,2);
mean_face = reshape(img_mean,r,c);
figure;
imshow(mean_face,[]);
title('Mean Face');
% subtract mean image from the set of images
img_shifted = imgmat-repmat(img_mean,1,M);

%perform PCA. Matrix imgmat was used as input instead of img_shifted because 
%Matlab documentation states that pca function centers the data
[coeff,score,latent] = pca(imgmat);
...
I_test = I_test-img_mean; % subtract mean images
